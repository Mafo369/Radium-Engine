...
void MinimalComponent::initialize() {
    ...
    */
    ...
    if ( ENABLE_SPHERES ) {
        ...
        for ( uint i = 0; i < numberOfSphere; ++i ) {
            ...
            Vector3 center { cellCorner +
                             Vector3 { cellSize / 2_ra, ratio * .1_ra, cellSize / 2_ra } };
    ...
    if ( ENABLE_DISKS ) {
        ...
        auto disk =
            RenderObject::createRenderObject( "test_disk",
                                              this,
                                              RenderObjectType::Geometry,
    ...
    auto addMesh = [this, colorBoost, plainMaterial]( Vector3 pos, TopologicalMesh topo1 ) {
        ...
        renderObject2->setLocalTransform( Transform { Translation( Vector3( pos ) ) *
                                                      Eigen::UniformScaling<Scalar>( 0.003_ra ) } );
    ...
    if ( ENABLE_SPLIT ) {
        ...
        auto addSplitScene = [findHalfedge, addMesh]( Vector3 pos,
                                                      const Vector3Array& points,
                                                      const Vector4Array& colors,
                                                      const Vector3uArray& indices,
                                                      Vector3 from,
